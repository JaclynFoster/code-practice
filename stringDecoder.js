...
// Each number tells you how many characters to skip before finding a good letter. After each good letter should come 
// the next next number.
// For example, the string ‘hey’ could be encoded to ‘0h2xce5ngbrdy’. This means ‘skip 0, find the ‘h’, skip 2, find the ‘e’, 
// skip 5, find the ‘y’.
...
const decoder = code => {
    ...
    console.log('decode Number:', splitString)
    let solution = ''
    for (let i = 0; i < splitString.length; i++) {
    if (!isNaN(+splitString[i])) {
        i += +splitString[i]
    } else {
        solution += splitString[i]
    }
    }
    return solution
}
